...
// gate below, so extra test dates can collect real data without appearing
...
// Sat 12 / Sun 13 Sep: not part of the monitored 11 weekdays, but the native
// GitHub Actions schedule now fires every day regardless (its day-of-week
// field is unrestricted) and was just being skipped on weekends by the
// TARGET_DATES check below. Temporarily allowing these two extra dates lets
// that same schedule exercise the full pipeline over the weekend as a
// reliability check ahead of Monday, with no manual triggering needed.
// Remove this once the weekend test is done.
const TEST_COLLECTION_DATES = ['2026-09-12', '2026-09-13'];
...
async function main() {
  ...
  if (!force) {
    if (!TARGET_DATES.includes(dateStr) && !TEST_COLLECTION_DATES.includes(dateStr)) {
      console.log(`${dateStr} is not one of the monitored weekdays (or weekend test dates). Skipping.`);
      return;
    }
